...
namespace flx {
	...
	GjkEpa::ClosestElement GjkEpa::getClosestInSegment(const Coordinate& A, const Coordinate& B, float* miximg_coeff) {
		miximg_coeff[2] = 0.f;
		Coordinate B_A = B;
		B_A.x -= A.x;
		B_A.y -= A.y;
	...
	GjkEpa::ClosestElement GjkEpa::getClosestInTriangle(const Coordinate& A, const Coordinate& B, const Coordinate& C, float* miximg_coeff) {
		Coordinate B_A = B;
		B_A.x -= A.x;
		B_A.y -= A.y;
		B_A.z -= A.z;
	...
	void GjkEpa::computeOutsideNormal(Coordinate& N, const Coordinate& P1, const Coordinate& P2, const Coordinate& P3, const Coordinate& Pother) {
		...
		if (dot(N, Delta1) > GEOMETRIC_TOLLERANCE) invert(N);
		normalizeInPlace(N);
	}
}
