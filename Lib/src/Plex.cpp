...
namespace flx {
	GjkEpa::Plex::Plex(GjkEpa& user, const ShapePair& pair
		...
		Array iterations;
#endif
		for (std::size_t k = 0; k < 4; ++k) this->vertices[k] = std::make_unique<MinkowskiCoordinate>();
		this->searchDirection = {1.f, 0.f, 0.f};
		this->user.getSupportMinkowskiDiff(this->pair, this->searchDirection, *this->vertices[1]);
	...
	static inline const std::uint8_t INCIDENCES[3][3] = {{0,1,2},
														 {0,1,3},
														 {0,2,3}};
	void GjkEpa::Plex::update4() {
		...
		for (std::uint8_t k = 0; k < 3; ++k) {
			if (this->Origin_is_visible[k]) {
				regions[k] = getClosestInTriangle(this->vertices[INCIDENCES[k][0]]->vertexDiff,
	...
	void GjkEpa::Plex::update3() {
		auto closest = getClosestInTriangle(this->vertices[0]->vertexDiff, this->vertices[1]->vertexDiff, this->vertices[2]->vertexDiff, this->coeff);
		Coordinate temp;
		mix3(temp, this->vertices[0]->vertexDiff, this->vertices[1]->vertexDiff, this->vertices[2]->vertexDiff, this->coeff);
		...
		case vertex_A:
			...
			break;
		default:
			if(edge_AB == closest) this->setToSegment(0);
	...
	void GjkEpa::Plex::update2() {
		...
		ClosestElement region = getClosestInSegment(this->vertices[0]->vertexDiff, this->vertices[1]->vertexDiff, this->coeff);
	...
	void GjkEpa::Plex::finishingLoop(CoordinatePair& closestPoints) {
		...
		if(2 == this->plex_dim) {
			getClosestInSegment(this->vertices[1]->vertexDiff, this->vertices[2]->vertexDiff, this->coeff);
			mix2(closestPoints.pointA, this->vertices[1]->vertexA, this->vertices[2]->vertexA, this->coeff);
			mix2(closestPoints.pointB, this->vertices[1]->vertexB, this->vertices[2]->vertexB, this->coeff);
			return;
		}
		getClosestInTriangle(this->vertices[1]->vertexDiff, this->vertices[2]->vertexDiff, this->vertices[3]->vertexDiff, this->coeff);
		mix3(closestPoints.pointA, this->vertices[1]->vertexA, this->vertices[2]->vertexA, this->vertices[3]->vertexA, this->coeff);
		mix3(closestPoints.pointB, this->vertices[1]->vertexB, this->vertices[2]->vertexB, this->vertices[3]->vertexB, this->coeff);
	}
	...
	void GjkEpa::Plex::setToFacet(const std::uint8_t& kind) {
		...
		else this->searchDirection = this->Normals[kind];
		this->plex_dim = 3;
	}
