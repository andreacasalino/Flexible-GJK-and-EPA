...
namespace flx {	
	GjkEpa::Epa::Epa(GjkEpa::Plex& lastPlex, CoordinatePair& result
		...
		std::array<GjkEpa::MinkowskiCoordinate, 4> tetra = this->getInitialTethraedron(lastPlex);
		hull::Hull Minkowski_diff(tetra[0].vertexDiff,
								  tetra[1].vertexDiff,
								  tetra[2].vertexDiff,
								  tetra[3].vertexDiff,this);
		{
			auto it = Minkowski_diff.getVertices().begin();
			originalVertices.emplace(&(*it), MinkowskiPair(tetra[0].vertexA, tetra[0].vertexB));
			++it;
			originalVertices.emplace(&(*it), MinkowskiPair(tetra[1].vertexA, tetra[1].vertexB));
			++it;
			originalVertices.emplace(&(*it), MinkowskiPair(tetra[2].vertexA, tetra[2].vertexB));
			++it;
			originalVertices.emplace(&(*it), MinkowskiPair(tetra[3].vertexA, tetra[3].vertexB));
		}
		...
		do {
			...
			user.getSupportMinkowskiDiff(lastPlex.getPair(), searchDirection, newVertex);
	...
	std::array<GjkEpa::MinkowskiCoordinate, 4> GjkEpa::Epa::getInitialTethraedron(GjkEpa::Plex& lastPlex) {
		...
		{	
			...
			for(std::uint8_t k=0; k<size; ++k) initialVertices[k] = *v[1 + k].get();
		}
		while (size < 4) {
			if (1 == size) {
				...
				else  invert(searchDirection);
				lastPlex.getUser().getSupportMinkowskiDiff(lastPlex.getPair(), searchDirection, initialVertices[1]);
				if(squaredDistance(initialVertices[0].vertexDiff, initialVertices[1].vertexDiff) <= GEOMETRIC_TOLLERANCE2) {
				...
				searchDirection = Coordinate{1.f, 0.f, 0.f};
				...
				lastPlex.getUser().getSupportMinkowskiDiff(lastPlex.getPair(), searchDirection, initialVertices[2]);
				bool tempB[2];
				tempB[0] = (squaredDistance(initialVertices[2].vertexDiff, initialVertices[0].vertexDiff) <= GEOMETRIC_TOLLERANCE2);
				tempB[1] = (squaredDistance(initialVertices[2].vertexDiff, initialVertices[1].vertexDiff) <= GEOMETRIC_TOLLERANCE2);
				if (tempB[0] || tempB[1]) {
					invert(searchDirection);
					lastPlex.getUser().getSupportMinkowskiDiff(lastPlex.getPair(), searchDirection, initialVertices[2]);
				}
				...
				invert(searchDirection);
				lastPlex.getUser().getSupportMinkowskiDiff(lastPlex.getPair(), searchDirection, initialVertices[3]);
				Coordinate Delta;
	...
	void GjkEpa::Epa::AddedChangedFacets(const std::list<const hull::Facet*>& added
									,const std::list<const hull::Facet*>& changed) const {
		auto it = added.begin();
		for(it; it!=added.end(); ++it) this->facetDistances.emplace(*it, dot((*it)->N, *(*it)->A));
		for(it = changed.begin(); it!=changed.end(); ++it)  this->facetDistances.find(*it)->second = dot((*it)->N, *(*it)->A);			
	};

	void GjkEpa::Epa::RemovedFacets(const std::list<const hull::Facet*>& removed) const {
		auto itEnd = removed.end();
		for(auto it = removed.begin(); it!=itEnd; ++it) this->facetDistances.erase(this->facetDistances.find(*it));
	};

#ifdef FLX_LOGGER_ENABLED
	std::string GjkEpa::Epa::print(const hull::Hull& h, const hull::Facet& closest) const {
		std::stringstream ss;
		ss << '{';
		ss << "\"facets\":";
		{
